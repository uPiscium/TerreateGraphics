...
void Icon::LoadImage(std::string imagePath) {
    GLFWimage image;
    unsigned char *ptr =
        stbi_load(imagePath.c_str(), &image.width, &image.height, NULL, 4);
    mImagePointers.push_back(ptr);
    image.pixels = mImagePointers[mImagePointers.size() - 1];
}

void Icon::LoadPixels(unsigned width, unsigned height,
...
Cursor::Cursor(unsigned xHot, unsigned yHot, std::string imagePath)
    : mXHot(xHot), mYHot(yHot) {
    mImagePointer =
        stbi_load(imagePath.c_str(), &mImage.width, &mImage.height, NULL, 4);
...
void Cursor::LoadPixels(unsigned width, unsigned height,
                        std::vector<unsigned char> bitmap) {
    mImageData = bitmap;
    mImage.width = width;
...
WindowSettings::WindowSettings(int resizable, int visible, int decorated,
                               int focused, int autoIconify, int floating,
                               int maximized, int centerCursor,
                               int transparentFramebuffer, int focusOnShow,
                               int scaleToMonitor)
    : mResizable(resizable), mVisible(visible), mDecorated(decorated),
      mFocused(focused), mAutoIconify(autoIconify), mFloating(floating),
      mMaximized(maximized), mCenterCursor(centerCursor),
      mTransparentFramebuffer(transparentFramebuffer),
...
Window::Window(GLsizei width, GLsizei height, std::string title,
               WindowSettings settings, Monitor monitor)
    : mSettings(settings) {
    settings.Apply();
    mWindow = glfwCreateWindow(width, height, title.c_str(),
...
Window::SetRefreshCallback(WindowRefreshCallback callback) {
...
Window::SetIconifyCallback(WindowIconifyCallback callback) {
...
Window::SetMaximizeCallback(WindowMaximizeCallback callback) {
...
Window::SetFramebufferSizeCallback(FramebufferSizeCallback callback) {
...
Window::SetContentScaleCallback(ContentScaleCallback callback) {
...
CursorPosCallback
Window::SetCursorPositionCallback(CursorPosCallback callback) {
...
CursorEnterCallback
Window::SetCursorEnterCallback(CursorEnterCallback callback) {
...
void *GetUserPointer(Window &window) {
    PointerWrapper *ptr =
        (PointerWrapper *)glfwGetWindowUserPointer((GLFWwindow *)window);
