...
class Attribute {
  ...
  public:
    Attribute()
        : mType(GLType::FLOAT), mComponents(-1), mStride(-1), mOffset(-1) {
        ;
    }
    /*
    ...
    */
    Attribute(GLType type, GLsizei components, GLsizei stride, GLsizei offset)
        : mType(type), mComponents(components), mStride(stride),
          mOffset(offset) {
        ;
    }
    /*
...
template <typename T> class VertexBuffer {
  ...
  public:
    ...
    * ELEMENT_BUFFER can be used as index buffer.
    */
    VertexBuffer(BufferType type, const std::vector<T> &data,
    ...
    */
    void SetBufferData(const std::vector<T> &data) {
        ...
        glBufferData((GLenum)mBufferType, this->GetSize(), &data[0],
                     (GLenum)mUsage);
    ...
    */
    void UpdateBufferData(const std::vector<T> &data, size_t offset = 0) {
        glBindBuffer((GLenum)mBufferType, mID);
        glBufferSubData((GLenum)mBufferType, offset * mComponentSize,
                        data.size() * mComponentSize, &data[0]);
        glBindBuffer((GLenum)mBufferType, 0);
    }
    /*
     * Bind all buffers.
...
class VertexArray {
  ...
  private:
    ...
    GetAttributeDatas(const std::vector<Attribute> &attrs);
  ...
  public:
    ...
    */
    template <typename T> void Register(const VertexBuffer<T> &buffer) {
        ...
        if (attrs.size() != 0) {
            ...
            for (unsigned i = 0; i < attrs.size(); i++) {
                ...
                glVertexAttribPointer(i, attr.mComponents, (GLenum)attr.mType,
    ...
    * mode : Draw mode of OpenGL. {Default : DrawMode::TRIANGLES}
    [notes]
    * If you want to draw the same object many times, "DrawInstances()" method
    is more suited.
    */
    ...
    * mode : Draw mode of OpenGL. {Default : DrawMode::TRIANGLES}
    */
    void DrawInstances(size_t numOfInstances,
                       DrawMode mode = DrawMode::TRIANGLES);
};
} // namespace GeoFrame

std::ostream &operator<<(std::ostream &stream,
                         const GeoFrame::Attribute &attribute);
