...
class Icon {
  ...
  public:
    ...
    */
    void LoadPixels(unsigned width, unsigned height,
  ...
  public:
    ...
    */
    void LoadStandard(CursorType type) {
        mCursor = glfwCreateStandardCursor((int)type);
    }
...
class WindowSettings {
  ...
  public:
    ...
    * Window hints.
    [params]
    * resizable : Whether the windowed mode window will be resizable by the
    user. {Default : GLFW_TRUE}
    ...
    * decorated : Whether the windowed mode window will have window decorations.
    {Default : GLFW_TRUE}
    * focused : Whether the windowed mode window will be given input focus when
    created. {Default : GLFW_TRUE}
    * autoIconify : Whether the full screen window will automatically iconify
    and restore the previous video mode on input focus loss. {Default :
    GLFW_TRUE}
    * floating : Whether the windowed mode window will be floating above other
    regular windows, also called topmost or always-on-top. {Default :
    GLFW_FALSE}
    * maximized : Whether the windowed mode window will be maximized when
    created. {Default : GLFW_FALSE}
    * centerCursor : Whether the cursor should be centered over newly created
    full screen windows. {Default : GLFW_TRUE}
    * transparentFramebuffer : Whether the window framebuffer will be
    transparent. {Default : GLFW_FALSE}
    * focusOnShow : Whether the window will be given input focus when
    glfwShowWindow() is called. {Default : GLFW_TRUE}
    * scaleToMonitor : Whether the window content area should be resized based
    on the monitor content scale of any monitor it is placed on. {Default :
    GLFW_FALSE}
    *
    [See also]
    * https://www.glfw.org/docs/latest/window_guide.html
    */
    WindowSettings(int resizable = GLFW_TRUE, int visible = GLFW_TRUE,
                   int decorated = GLFW_TRUE, int focused = GLFW_TRUE,
                   int autoIconify = GLFW_TRUE, int floating = GLFW_FALSE,
                   int maximized = GLFW_FALSE, int centerCursor = GLFW_TRUE,
  ...
  public:
    ...
    */
    int GetInputMode(InputType inputType) const {
        return glfwGetInputMode(mWindow, (int)inputType);
    }
    /*
    * Get window size.
    [returns]
    ...
    */
    std::pair<int, int> GetAspectRatio() const {
        return this->GetPropertyPointer()->mAspectRatio;
    }
    /*
    ...
    */
    bool GetMousebutton(MousebuttonInput button) const {
        return (bool)glfwGetMouseButton(mWindow, (int)button);
    }
    /*
    * Get cursor position.
    [returns]
    ...
    */
    std::pair<float, float> GetScrollOffset() const {
        return this->GetPropertyPointer()->mScrollOffset;
    }
    /*
    ...
    */
    std::vector<std::string> GetDroppedFiles() const {
        return this->GetPropertyPointer()->mDroppedFiles;
    }
    /*
    * Get window title.
    [returns]
    ...
    */
    std::vector<Key> GetKeys() const {
        return this->GetPropertyPointer()->mKeyInputs;
    }
    /*
    ...
    */
    std::vector<unsigned> GetChars() const {
        return this->GetPropertyPointer()->mCharInputs;
    }
    /*
    * Get window opacity.
    [returns]
    ...
    */
    void GetPixels(int x0, int y0, int width, int height, PixelFormat format,
                   GLType type, void *pixels) {
        glReadPixels(x0, y0, width, height, (int)format, (int)type, pixels);
    }
    ...
    */
    void SetIcon(Icon &icon) {
        glfwSetWindowIcon(mWindow, icon.GetNumImages(), (GLFWimage *)icon);
    }
    /*
    ...
    * Set framebuffer size callback function.
    [params]
    * callback : Framebuffer size callback. Called when framebuffer size is
    changed.
    */
    ...
    * Set cursor position callback function.
    [params]
    * callback : Cursor position callback. Called when cursor position is
    changed.
    */
    ...
    */
    bool IsPressing(Keyboard key) const {
        return bool(glfwGetKey(mWindow, (GLenum)key));
    }
